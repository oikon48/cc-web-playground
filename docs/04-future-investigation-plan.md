...
```bash
# セッション情報確認
echo $CLAUDE_CODE_CONTAINER_ID
cat /container_info.json

# アイドル時のタイムアウト測定
# → 一定時間操作せずに放置し、切断タイミングを記録

# 複数タブテスト
# → 別ブラウザタブで同じプロジェクトを開き、挙動を確認
```
...
**目的**: サービスの利用制限を把握し、制限内で最適化する

#### 調査項目

- [ ] **API呼び出しレート制限**
  - Claude Code API（存在する場合）
  - GitHub API（プロキシ経由）
  - その他外部API
...
# プロセス数テスト
for i in {1..1000}; do sleep 100 & done
ps aux | wc -l

# ファイル書き込みストレステスト
for i in {1..10000}; do echo "test" > /tmp/file_$i; done
...
| 調査項目 | 推定工数 | 依存関係 | 備考 |
|---------|---------|---------|------|
| **2. セッション管理とライフサイクル** | 4h | なし | タイムアウトで作業ロス防止 |
| **6. データ永続化の境界線** | 3h | なし | データ損失リスク回避 |
| **7. レート制限とクォータ** | 2h | なし | サービス停止回避 |

**目標**: これらを完了すれば、安心して実用できる
