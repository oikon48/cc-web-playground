...
- `"Bash(gh :*)"` - ghで始まるすべてのBashコマンドを許可
- `:*` はプレフィックスマッチング（必須）
- 他の例: `"Bash(git:*)"`, `"Bash(npm run:*)"`
...
**メリット**:
- ✅ 次回セッションから`gh`コマンドが自動承認される
- ✅ チーム全体で共有可能（Gitにコミット）
- ✅ 承認プロンプトなしで実行可能

**注意**: 設定を有効にするには**セッション再起動（ブラウザリロード）が必要**
